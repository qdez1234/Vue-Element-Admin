<template>
    <el-dialog v-model="dialogTableVisible" :before-close="handleClose" title="导入配置" width="80%" draggable>
        ...
        <div class="ex-table-page-body">
            <AgTable class="AgTable" ref="gridRef" :gridColDefs="gridColDefs" :suppressRowClickSelection="false" :paging="false" :gridKey="route.path"
                @handleCellSelect="handleCellSelect" @HandleLinkTextClick="HandleLinkTextClick" />
        </div>
        <template #footer>
...
const handleOpen = (config) => {
    let { data, model, rowSelection } = config
...
// 表格提交
const handleSubmit = () => {
    emit('update:data', { model: selectModel.value, data: getData(), rowSelection: selectRowSelection.value })
    handleClose()
};
// 获取全部数据
const getData = () => {
