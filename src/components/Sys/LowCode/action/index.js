...
export let $action = {
    ...
    getBusinessModelCode() {
        ...
        let businessModelCode = route.meta.isMain == '1' ? route.meta.businessModelCode : route.query.businessModelCode
     ...
     * @param {} data 
     */
    async HandleGet(data, callback = function () { }) {
        let url = commonDetailAPI + this.getBusinessModelCode();
        const res = await this.HandleActionAPI(url, data);  // 调用异步API函数
        if (res.code == 200) {
            callback()
        } return res
     ...
     * @param {} data 
     */
    async HandleAdd(data, callback = function () { }) {
        let url = commonAddAPI + this.getBusinessModelCode();
        const res = await this.HandleActionAPI(url, data);  // 调用异步API函数
        if (res.code == 200) {
            callback()
            ElMessage.success(res.msg)
     ...
     * @param {} data 
     */
    async HandleChange(data, callback = function () { }) {
        let url = commonChangeAPI + this.getBusinessModelCode();
        const res = await this.HandleActionAPI(url, data);  // 调用异步API函数
        if (res.code == 200) {
            callback()
            ElMessage.success(res.msg)
