    ...
    <div class="center">
      <div class="page-card-box">
        <el-tabs type="border-card" class="card" v-model="pageLabel" @tab-click="handleTabsClick">
          <el-tab-pane label="列表页" name="main"></el-tab-pane>
          <el-tab-pane label="详情页" name="detail"></el-tab-pane>
        </el-tabs>
          ...
          <draggable v-model="drawingListSet" group="componentsGroupContainer" class="drawing-board">
            <template #item="{ element: outerElement }">
              <div class="template">
                ...
                <!-- 弹窗start -->
                <div :class="['draggableContainer', activedId == outerElement.id ? 'activedDraggable' : '']"
                  @click="handleSelect(outerElement)" v-if="outerElement.modelType == 'ex-dialog'">
                  <div class="floatIcon" @click="handleDelete(outerElement.id)">
                    <i :class="['iconfont', 'icon-shanchu']"></i>
                  </div>
                  <el-form class="ex-form-inline" :inline="true" :label-width="outerElement.labelWidth"
                    ...
                    <draggable v-model="outerElement.children" group="form" class="draggableChild"
                      ...
                      <template #item="{ element }">
                        <el-tab-pane :label="element.label" :name="element.id" v-if="element.modelType == 'ex-table'">
                          <div class="floatIcon" @click="handleDelete(element.id)">
                            <i :class="['iconfont', 'icon-shanchu']"></i>
                          </div>
...
import { getDataBaseName, uploadDataBaseList } from "@/api/system/config";
import { commonSearchAPI } from "../../config/index.js"
import { v4 as uuidv4 } from "uuid";
const route = useRoute()
const router = useRouter()
const drawingListSet = ref([])
const mainConfigSet = ref([])
const detailConfigSet = ref([])
...
watch(() => drawingListSet.value, (newVal) => {
  pageName == 'main' ? mainConfigSet.value = newVal : ''
  pageName == 'detail' ? detailConfigSet.value = newVal : ''
}, { deep: true })

const handleTabsClick = (ele) => {
  pageName = ele.props.name
  switch (pageName) {
    case 'main':
      drawingListSet.value = mainConfigSet.value
      break;
    case 'detail':
      drawingListSet.value = detailConfigSet.value
      break;
    default:
      drawingListSet.value = mainConfigSet.value
  }
 ...
 * 保存数据
 */
const handleSave = (e, back = false) => {
  pageConfig.value.source = JSON.stringify({ main: mainConfigSet.value || [], detail: detailConfigSet.value || [] })
  uploadDataBaseList(pageConfig.value).then(res => {
...
const handleInit = () => {
  getDataBaseName(route.query.businessModelCode).then(res => {
    pageConfig.value = res.data
    if (res.data.source) {
      mainConfigSet.value = (JSON.parse(res.data.source)).main
      detailConfigSet.value = (JSON.parse(res.data.source)).detail
